...
class RollershutterSubscriber(object):
    ...
    def __init__(self, item_name):
        ...
        # callback function.
        self.sub = rospy.Subscriber("/openhab/items/" + self.item_name + "/state", RollershutterState, self.callback)
...
if __name__ == '__main__':
    ...
    node_name = "RollershutterSubscriberNode"
    rospy.init_node(node_name, anonymous=True)

    rollershutterSubscriber = RollershutterSubscriber("testRollershutter")

    # Go to the main loop
    try:
        rollershutterSubscriber.start()
        # Wait for messages on topic, go to callback function when new messages arrive.
        # spin() simply keeps python from exiting until this node is stopped
        rospy.spin()
    # Stop with Ctrl + C
    except KeyboardInterrupt:
        rollershutterSubscriber.stop()
